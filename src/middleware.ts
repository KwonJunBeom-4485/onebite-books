// middleware.ts는 /src/ 바로 밑에 있어야 한다.
// next.config.ts 와 같은 위치가 아님..

import { NextRequest, NextResponse } from "next/server";

// Edge Runtime 에서 동작하기 때문에 Node.js API(fs, crypto 등) 사용 불가
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/admin')) {
    ...
    // adminToken이 없으면 기존 경로로 되돌리기(redirect)
    if (!adminToken) {
...
// matcher: 미들웨어를 적용할 경로 패턴 지정할 수 있음.
// matcher에 포함되지 않는 경로는 미들웨어를 거치지 않습니다.
// 정적 파일(_next/static 등)은 자동으로 제외
export const config = {
